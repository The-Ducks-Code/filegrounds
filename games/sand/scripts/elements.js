...
const METAL_WALL = __inGameColor(211, 211, 211);
...
const STEAMER = __inGameColor(100, 100, 200);
const WELL = __inGameColor(131, 11, 28);
const TORCH = __inGameColor(200, 5, 0);
const GUNPOWDER = __inGameColor(170, 170, 140);
const NUCLEAR_FLUID = __inGameColor(5, 255, 5);
const ULTRA_NUKE_FLUID = __inGameColor(255, 0, 0);
...
const STEAMNOWATER = __inGameColor(195, 214, 235);
...
const VIRUS = __inGameColor(23, 16, 54);
...
const elementActions = [
  ...
  TORCH_ACTION,
  GUNPOWDER_ACTION,
  NUCLEAR_FLUID_ACTION,
  ULTRA_NUKE_FLUID_ACTION,
  WAX_ACTION,
  FALLING_WAX_ACTION,
  NITRO_ACTION,
  NAPALM_ACTION,
...
function initElements() {
  ...
  GAS_PERMEABLE[NUCLEAR_FLUID] = null;
  GAS_PERMEABLE[ULTRA_NUKE_FLUID] = null;
...
function WALL_ACTION(x, y, i) {
  if (bordering(x, y, i, VIRUS) !== -1) {
    gameImagedata32[i] = VIRUS;
  }
}
function METAL_WALL_ACTION(x, y, i) {}
...
function STEAMER_ACTION(x, y, i) {
  doProducer(x, y, i, STEAMNOWATER, false, 100);
  if (bordering(x, y, i, VIRUS) !== -1) {
    gameImagedata32[i] = VIRUS;
  }
}
...
function NUCLEAR_FLUID_ACTION(x, y, i) {
  if (bordering(x, y, i, FIRE) !== -1) {
    if (!particles.addActiveParticle(NUKE_PARTICLE, x, y, i)) {
      gameImagedata32[i] = FIRE;
    }
    return;
  }else if (bordering(x, y, i, VIRUS) !== -1) {
    gameImagedata32[i] = VIRUS;
  } else if (doGravity(x, y, i, true, 95)) return;
  if (bordering(x, y, i, CHARGED_NITRO) !== -1) {
    gameImagedata32[i] = ULTRA_NUKE_FLUID;
  }else if (bordering(x, y, i, ULTRA_NUKE_FLUID) !== -1) {
    gameImagedata32[i] = ULTRA_NUKE_FLUID;
  } if (doGravity(x, y, i, true, 95)) return;
}
function ULTRA_NUKE_FLUID_ACTION(x, y, i) {
  if (random() < 25 && bordering(x, y, i, FIRE) !== -1) {
    if (!particles.addActiveParticle(ULTRA_NUKE_PARTICLE, x, y, i)) {
      gameImagedata32[i] = FIRE;
    }
    return;
  }
...
function STEAMNOWATER_ACTION(x, y, i) {
  if (doDensityGas(x, y, i, 70)) return;
  if (doRise(x, y, i, 70, 60)) return;
  if (bordering(x, y, i, VIRUS) !== -1) {
    gameImagedata32[i] = VIRUS;
...
function MYSTERY_ACTION(x, y, i) {
  ...
  if (bordering(x, y, i, POLLEN) !== -1) {
    ...
    if (bordering(x, y, i, STEAM) !== -1) {
      particles.addActiveParticle(TREE_PARTICLE, x, y, i);
      gameImagedata32[i] = BACKGROUND;
      return;
    }else if (bordering(x, y, i, VIRUS) !== -1) {
      gameImagedata32[i] = VIRUS;
...
function CHARGED_NITRO_ACTION(x, y, i) {
  ...
  if (bordering(x, y, i, NUCLEAR_FLUID) !== -1) {
    gameImagedata32[i] = ULTRA_NUKE_FLUID;
  }
  if (bordering(x, y, i, ULTRA_NUKE_FLUID) !== -1) {
    gameImagedata32[i] = ULTRA_NUKE_FLUID;
  }
  if (bordering(x, y, i, VIRUS) !== -1) {
    gameImagedata32[i] = VIRUS;
...
function __doBigExplosion(x, y, i) {
  const burn = random() < 150;
  const replace = burn ? FIRE : NUCLEAR_FLUID;
  const isNotLeftmost = x !== 0;
  const isNotRightmost = x !== MAX_X_IDX;
  ...
  if (y - 2 >= 0) {
    const twoUp = i - 2 * width;
    if (gameImagedata32[twoUp] !== NUCLEAR_FLUID || random() < 500)
      gameImagedata32[twoUp] = FIRE;
  }
  if (y + 2 >= 0) {
    const twoDown = i + 2 * width;
    if (gameImagedata32[twoDown] !== NUCLEAR_FLUID || random() < 500)
      gameImagedata32[twoDown] = FIRE;
  }
  if (x - 2 >= 0) {
    const twoLeft = i - 2;
    if (gameImagedata32[twoLeft] !== NUCLEAR_FLUID || random() < 500)
      gameImagedata32[twoLeft] = FIRE;
  }
  if (x + 2 >= 0) {
    const twoRight = i + 2;
    if (gameImagedata32[twoRight] !== NUCLEAR_FLUID || random() < 500)
      gameImagedata32[twoRight] = FIRE;
  }
}
