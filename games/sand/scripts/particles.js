...
function ULTRA_NUKE_PARTICLE_INIT(particle) {
  particle.setColor(FIRE);
  const maxDimension = Math.max(width, height);
  particle.size = maxDimension / 1 + (Math.random() * maxDimension) / 1;
}

function ULTRA_NUKE_PARTICLE_ACTION(particle) {
  particle.drawCircle(particle.size);
  if (particle.actionIterations > 15) particles.makeParticleInactive(particle);
...
function initParticles() {
  ...
  PAINTABLE_PARTICLE_COLORS[METAL_WALL] = null;
